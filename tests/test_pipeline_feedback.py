...
def test_pipeline_retry_limit_advances_pipeline(monkeypatch):
    pipeline = ALG2SVPipeline()
    stage_outputs = _default_stage_outputs()

    verify_failure = VerifyResults(
        tests_total=0,
        tests_passed=0,
        all_passed=False,
        mismatches=[],
    ...
    stage_outputs['verify'] = deque([verify_failure, verify_failure, verify_failure])

    stage_outputs['synth'] = deque([
        SynthResults(
            fmax_mhz=220.0,
            timing_met=True,
            lut_usage=1400,
            ...
            total_power_mw=50.0,
            slack_ns=0.5,
            reports_path="synth/success",
        )
    ])

    feedback_decisions = deque([
        {"action": "continue"},  # spec
        {"action": "continue"},  # quant
        {"action": "continue"},  # microarch
        {"action": "continue"},  # rtl
        {"action": "retry_verify", "guidance": "No tests executed."},
        {"action": "retry_verify"},
        {"action": "retry_verify"},
        {"action": "continue"},  # synth
        {"action": "continue"},  # lint
        {"action": "continue"},  # simulate
    ...
    assert result["success"] is True
    assert pipeline.stage_attempts['verify'] == pipeline.MAX_STAGE_ATTEMPTS
    assert stage_calls['verify'] == pipeline.MAX_STAGE_ATTEMPTS
    assert pipeline.results['verify'].tests_total == 0
    assert stage_calls['synth'] == 1
