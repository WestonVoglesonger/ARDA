...
def _make_runner() -> OpenAIAgentRunner:
    return OpenAIAgentRunner.__new__(OpenAIAgentRunner)


def test_extract_payload_from_output_json():
    runner = _make_runner()
    response = _Stub(output=[_Stub(output_json={"foo": 1})])
    payload = runner._extract_response_payload(response, "rtl")
...
def test_extract_payload_from_content_json_dict():
    ...
    payload = runner._extract_response_payload(response, "quant")
...
def test_attribute_json_on_block_is_ignored():
    ...
    payload = runner._extract_response_payload(_Stub(output=[Block()]), "spec")
...
def test_extract_payload_from_callable_json_accessor():
    ...
    payload = runner._extract_response_payload(response, "microarch")
...
def test_extract_payload_from_json_string_text():
    ...
    payload = runner._extract_response_payload(response, "spec")
...
def test_extract_payload_invalid_text_raises():
    ...
    with pytest.raises(RuntimeError):
        runner._extract_response_payload(response, "rtl")
